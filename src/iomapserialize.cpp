...
bool IOMapSerialize::loadMapRelational(Map* map)
{
	...
	for(HouseMap::iterator it = Houses::getInstance().getHouseBegin(); it != Houses::getInstance().getHouseEnd(); ++it){
		...
		//[[--Darghos
		if(house->getPendingExpropriation())
...
bool IOMapSerialize::loadMapBinary(Map* map)
{
	...
	do {
		...
		//[[--Darghos
		if(house && house->getPendingExpropriation())
			house->setHouseOwner(0);
		//--]]
	} while(result->next());
...
bool IOMapSerialize::loadHouseInfo(Map* map)
{
	...
	do{
		...
		if(house){
			...
			int32_t clear = result->getDataInt("clear");
			...
			//[[--Darghos
			if((clear & HOUSE_CLEAN_TRANSFER_ITEMS) != 0)
				house->setPendingDepotTransfer(true);

			if((clear & HOUSE_CLEAN_EXPROPRIATE_OWNER) != 0){
				if(!house->getPendingDepotTransfer())
					house->setHouseOwner(0);
				else
					house->setPendingExpropriation(true);
			}
			//--]]
		}
	}while(result->next());
