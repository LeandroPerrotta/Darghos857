...
bool CreatureEvent::configureEvent(xmlNodePtr p)
{
	...
	if(readXMLString(p, "type", str)){
		...
		//[[--Darghos
		else if(asLowerCaseString(str) == "combat"){
...
//[[--Darghos
uint32_t CreatureEvent::executeOnCombat(Creature* creature, Creature* target)
{
	//onCombat(cid, target)
	if(m_scriptInterface->reserveScriptEnv()){
		ScriptEnviroment* env = m_scriptInterface->getScriptEnv();
		...
		std::cout << "[Error] Call stack overflow. CreatureEvent::executeOnCombat" << std::endl;
		return 0;
	}
}
//--]]
