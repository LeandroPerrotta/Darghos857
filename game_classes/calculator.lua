...
function Calculator.getExpLeft(currentLevel, desiredLevel)
	return Calculator.getLevelExp(desiredLevel) - Calculator.getLevelExp(currentLevel)
end
...
function Calculator.getExpGainPercent(creatureExp, currentLevel)
	return (100 * creatureExp) / (Calculator.getExpLeft(currentLevel, currentLevel + 1))
end
...
function Calculator.getCreatureLeft(currentLevel, desiredLevel, creatureExp)
	return (Calculator.getExpLeft(currentLevel, desiredLevel) / getConfigInfo(rateExp)) * (1 / creatureExp)
end
...
function Calculator.getCreatureExpPercent(level, creatureExp)
	local levelExp = Calculator.getLevelExp(level)
...
function Calculator.getCapStats(cid, level, vocation)
	return (getPlayerFreeCap(cid) / Calculator.getCap(level, vocation)) * 100
end
...
function Calculator.getEnforcedExpGain(killerLevel, victimLevel)
	return (1 - (math.floor(killerLevel * 0.9) / victimLevel)) * 0.05 * Calculator.getLevelExp(victimLevel)
end
